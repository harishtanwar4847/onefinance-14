...
def reminders_at_six():
    ...
    payment_entry_name_without_approval = frappe.db.sql("""select p.approver,p.paid_amount,case when p.party_name is not null then p.party_name else p.paid_to end,p.remarks,p.reference_date from `tabPayment Entry` p where p.submitted_date = CURDATE() and p.submitted_date = p.amended_from_submitted_date and (not exists(select r.reference_doctype from `tabPayment Entry Reference` r where p.name=r.parent) or exists(select r.reference_doctype from `tabPayment Entry Reference` r where p.name=r.parent and (r.reference_doctype != "Purchase Invoice" and r.reference_doctype != "Purchase Order" and r.reference_doctype != "Journal Entry"))) and p.workflow_state = "Submitted" and p.payment_type != 'Receive' """)
    ...
    frappe.sendmail(subject="MIS Report - Bank Payment", content=msg_today, recipients = '{},{},{},{},{}'.format("jeet@1finance.co.in", "keval@1finance.co.in","mohan@1finance.co.in","accounts@1finance.co.in","harish.tanwar@atriina.com"))
    ...
    frappe.sendmail(subject="MIS Report - Credit Card", content=msg4_today, recipients = '{},{},{},{},{}'.format("jeet@1finance.co.in", "keval@1finance.co.in","mohan@1finance.co.in","accounts@1finance.co.in","harish.tanwar@atriina.com"))
