...
def on_update_purchase_invoice(doc, method):
    ...
    if not old_doc and frappe.db.exists("Purchase Invoice", {"supplier" : doc.supplier, "bill_no" : doc.bill_no, "workflow_state" : ["!=","Cancelled"]}):
...
def on_update_purchase_order(doc, method):
    old_doc = doc.get_doc_before_save()
    if not old_doc and frappe.db.exists("Purchase Order", {"supplier" : doc.supplier, "po_no" : doc.po_no, "workflow_state" : ["!=","Cancelled"]}):
